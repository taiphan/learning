...
from repo_paths import LAB, REPO
...
''')
    ...
    w(f"{lib}/sql/week05_queries.sql", open(LAB / "sql/week05_queries_solution.sql").read())
...
jobs:
  test:
    ...
    steps:
      ...
      - run: python lab/setup_db.py
      - run: python lab/projects/credit-pd-model/train.py
      - run: pytest lab/projects/week33_fastapi/test_health.py
      - run: python lab/exercises/week32_run_eval.py
""")
...
""")
    ...
    (LAB / "notebooks").mkdir(parents=True, exist_ok=True)
    (LAB / "notebooks" / "01_lending_kpis.ipynb").write_text(
        _json.dumps(nb01, indent=1), encoding="utf-8"
    )
    (LAB / "notebooks" / "02_eda_credit.ipynb").write_text(
        _json.dumps(nb02, indent=1), encoding="utf-8"
    )
    print("  wrote lab/notebooks/01_lending_kpis.ipynb")
    print("  wrote lab/notebooks/02_eda_credit.ipynb")

    print("\\nDone. Run: python lab/setup_db.py")
    print("Then: pip install -r lab/requirements.txt")
